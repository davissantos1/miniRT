...
static bool	intersection(t_circle *obj, double t, t_hit *hit, t_ray ray);
...
bool	hit_circle(t_scene *scene, void *me, t_hit *hit, t_ray ray)
{
	...
	if (fabs(t) <= 1e-7)
		return (false);
	return (intersection(obj, t, hit, ray));	
}

static inline bool	intersection(t_circle *obj, double t, t_hit *hit, t_ray ray)
{
	t_point	p;
	t_vec4	v;
	double distance;
