...
/*   Updated: 2026/02/09 20:16:36 by dasimoes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_camera(t_minirt *rt, char **entity)
{
	...
	if (!cam)
		desperation(rt, ERR_FILE_INVALID);
	cam->pos = vec4_init(ft_atod(p[0]), ft_atod(p[1]), ft_atod(p[2]), 0);
...
void	parse_light(t_minirt *rt, char **entity)
{
	...
	if (!lig)
		desperation(rt, ERR_FILE_INVALID);
	lig->pos = vec4_init(ft_atod(p[0]), ft_atod(p[1]), ft_atod(p[2]), 0);
	lig->color = vec4_init(ft_atod(c[0]), ft_atod(c[1]), ft_atod(c[2]), 0);
	lig->ratio = atod(entity[2]);
	rt->scene->light = lig;
	free(c);
	free(p);
}
